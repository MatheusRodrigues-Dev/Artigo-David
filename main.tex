...
\begin{abstract-en}
Este estudo apresenta uma avaliação heurística de três aplicativos móveis desenvolvidos para o ecossistema de automação residencial da Smart Sensor Design: Smart Energy Meter, Smart Flow Hall e Smart Fish Finder. A pesquisa foi conduzida em quatro etapas progressivas, envolvendo protótipos em Figma, versões beta em TestFlight, builds experimentais nas lojas Google Play e Apple App Store e, por fim, a análise de especialistas externos utilizando um checklist estruturado com base nas heurísticas de Nielsen. Os resultados revelam padrões recorrentes de usabilidade em sistemas IoT, destacando limitações relacionadas à visibilidade do estado do sistema, prevenção de erros, clareza terminológica e ausência de feedback informativo. Também foram identificados avanços implementados ao longo do processo iterativo de desenvolvimento, especialmente em consistência visual, navegação e comunicação de estados críticos. As contribuições principais incluem: (i) a sistematização de um modelo de avaliação aplicável a aplicativos IoT em desenvolvimento; (ii) a identificação de diretrizes práticas para reduzir a carga cognitiva dos usuários em cenários de monitoramento contínuo; e (iii) recomendações de design centradas em acessibilidade, personalização e tutoriais interativos, que podem apoiar desenvolvedores na construção de interfaces mais intuitivas e eficientes. Por fim, são discutidas limitações metodológicas e apresentadas direções para trabalhos futuros, incluindo testes em ambientes reais, análises longitudinais e o uso de IA para adaptação dinâmica de interfaces.
\end{abstract-en}

\begin{keywords}
	Usabilidade; Avaliação Heurística; Internet das Coisas (IoT); Experiência do Usuário (UX); Design de Interfaces; Nielsen.
\end{keywords}
...
Este estudo adotou o método de \textit{avaliação heurística} proposto por \cite{Nielsen1994}, técnica amplamente utilizada na área de Interação Humano-Computador (IHC) para identificar problemas de usabilidade por meio da inspeção sistemática da interface. A avaliação heurística consiste em confrontar as funcionalidades e elementos visuais de um sistema com um conjunto de dez princípios gerais de usabilidade, permitindo identificar violações, apontar inconsistências e priorizar melhorias. Trata-se de um método apropriado para produtos em desenvolvimento, especialmente em contextos de prototipação iterativa e sistemas IoT, cuja maturidade pode variar significativamente entre versões preliminares e builds funcionais \cite{Dix2004, Shneiderman2016}.

A pesquisa foi conduzida como um estudo de caso envolvendo três aplicativos móveis em desenvolvimento pela startup Smart Sensor Design (SSD), especializada em soluções baseadas em sensores e Internet das Coisas (IoT). Os aplicativos avaliados foram selecionados por contemplarem diferentes domínios de uso, energia elétrica, pesca e monitoramento hídrico, permitindo analisar como os princípios heurísticos se manifestam em contextos operacionais distintos.

\subsection{Descrição dos Aplicativos Avaliados}

\textbf{Smart Energy Meter:} aplicativo para monitoramento de consumo energético em tempo real, utilizando sensores indutivos conectados a um microcontrolador ESP32. O sistema apresenta registros de consumo, gráficos e indicadores que auxiliam o usuário na tomada de decisões relacionadas à eficiência energética.

\textbf{Smart Fish Finder:} aplicativo voltado ao monitoramento de atividade pesqueira por meio de sensores subaquáticos. Apresenta informações sobre localização de cardumes, profundidade e temperatura da água, oferecendo suporte à decisão para pescadores amadores e profissionais.

\textbf{Smart Flow Hall:} aplicativo destinado ao monitoramento de fluxo hídrico, empregando sensores baseados no efeito Hall. O sistema fornece dados de vazão, consumo acumulado e histórico de uso, visando apoiar o gerenciamento eficiente de recursos hídricos.

Esses aplicativos encontravam-se em diferentes níveis de maturidade: protótipos navegáveis em Figma, versões beta distribuídas via TestFlight e builds experimentais disponíveis em lojas restritas (\textit{Google Play} e \textit{Apple App Store}). Essa diversidade permitiu aplicar o método heurístico tanto em interfaces estáticas quanto em funcionalidades já implementadas, ampliando a abrangência da análise.

\subsection{Procedimento de Avaliação}

A avaliação heurística foi estruturada em quatro rodadas complementares, apresentadas na Figura~\ref{MetodoloEtapas}, cada uma com objetivos específicos e níveis distintos de fidelidade da interface:

\begin{figure}[!ht] \begin{center} \includegraphics[width=\columnwidth]{figures/metodologia/etapas.png} \caption{Etapas metodológicas da avaliação heurística} \label{MetodoloEtapas} \end{center} \end{figure}

\begin{enumerate}
    \item \textbf{Rodada 1 — Análise Inicial de Protótipos:} conduzida pelo pesquisador utilizando protótipos navegáveis em Figma e versões preliminares distribuídas via TestFlight. Essa etapa documentou o estado inicial das interfaces, identificando problemas estruturais de navegação, hierarquia da informação e compreensão dos dados.
    
    \item \textbf{Rodada 2 — Avaliação em Ambientes Próximos ao Uso Real:} o pesquisador analisou versões experimentais disponibilizadas em lojas restritas (\textit{Google Play} e \textit{Apple App Store}). Nessa rodada, foram observados comportamentos dinâmicos, tempos de resposta, funcionamento de gráficos e mensagens de sistema.
    
    \item \textbf{Rodada 3 — Revisão das Melhorias Implementadas:} consistiu na reaplicação do checklist após a implementação das correções sugeridas nas fases anteriores. O objetivo foi verificar a resolução das violações, identificar regressões e observar novos problemas decorrentes da evolução das funcionalidades.
    
    \item \textbf{Rodada 4 — Avaliação Independente por Especialistas:} realizada por dois especialistas externos com experiência em usabilidade e sistemas IoT. Os avaliadores aplicaram o mesmo checklist semiestruturado de forma independente, fornecendo validação técnica das etapas anteriores e contribuindo com observações adicionais não identificadas pelo pesquisador.
\end{enumerate}

Essa abordagem iterativa permitiu comparar versões, caracterizar problemas recorrentes e identificar padrões de usabilidade compartilhados entre os três aplicativos.

\subsection{Instrumentos e Registro dos Dados}

O instrumento principal utilizado foi um \textit{checklist} semiestruturado elaborado com base nos dez princípios heurísticos de \cite{Nielsen1994}. Cada heurística foi operacionalizada em um conjunto de critérios observáveis, permitindo maior padronização na coleta dos dados. Para cada problema identificado, o avaliador registrou:

\begin{itemize}
    \item a heurística violada;
    \item a descrição da ocorrência;
    \item a tela ou fluxo afetado;
    \item a evidência observada;
    \item o nível de gravidade (escala 0–4, conforme Nielsen).
\end{itemize}

Os registros das quatro rodadas foram tabulados em planilhas estruturadas e posteriormente analisados de forma comparativa. Essa análise permitiu calcular frequências, agrupar problemas por heurísticas e sintetizar a evolução dos aplicativos ao longo do processo iterativo.

\subsection{Aspectos Éticos}

A participação dos especialistas externos foi submetida e aprovada pelo Comitê Permanente de Ética em Pesquisa com Seres Humanos (COPEP/UEM), seguindo as diretrizes da Resolução CNS nº 466/2012. O projeto foi aprovado sob o número CAAE 73814323.6.0000.0104, Parecer 6.336.076.

Essa estrutura metodológica possibilitou uma avaliação robusta e sistemática das interfaces em desenvolvimento, combinando a familiaridade interna do pesquisador com a análise crítica de avaliadores externos experientes.
...
O diagnóstico inicial evidenciou lacunas estruturais que impactavam diretamente a compreensibilidade, previsibilidade e segurança de uso do aplicativo. Esses resultados forneceram insumos essenciais para orientar as intervenções nas etapas subsequentes, permitindo que os ajustes se concentrassem nos aspectos de maior gravidade e nos princípios heurísticos mais comprometidos. 

Frase para conectar da Primeira a Segunda e terceira Etapa
